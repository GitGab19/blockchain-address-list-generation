...
def main():
    ...
    while start_block < end_block:
        counter +=1
        time.sleep(0.1)
        endpoint = "https://api-eu1.tatum.io/v3/dogecoin/block/"+str(start_block)
        response = requests.get(endpoint, headers=headers)
        data = response.json()
        block = data
        percentage = (start_block-initial_block)/(end_block-initial_block)*100
        transactions_list = block['txs']
        print(str(block['height'])+ '---> ' + str(round(percentage,2)) + '%')
        with open('./doge_last_block.txt', 'w') as f:
